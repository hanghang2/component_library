		...
		<div class="tableDiv">
			<table border="" cellspacing="" cellpadding="">
				...
				<tr v-for="(item,index) in tableData" v-bind:class="{active: checkedArr[index]}" :key="index">
					...
					<!--其它行-->
					<td v-for="(itemAttr,index2) in tableheader" :key="index2">
						<div v-if="itemAttr.field" class="table-cell" v-show="!editShow[index]"
						     v-text="item[itemAttr.field]"></div>
						<input v-if="itemAttr.field" v-show="edit(editShow[index])" v-bind:disabled="!itemAttr.edit"
						       class="input-table"
						       v-model="item[itemAttr.field]"/>
					</td>
					<!--右漂浮-->
					<td v-for="(itemAttr,index) in tableheader" :key="'edit' + index">
						<div v-if="itemAttr.slot && (itemAttr.fixed == 'right')" class="table-cell-fixed"
						     v-bind:style="{width:itemAttr.width*1 + 2 + 'px'}">
							<slot :name="itemAttr.slot" :row="item" :i="index" :editshow="editShow"></slot>
						</div>
						<div v-if="itemAttr.slot" class="table-cell">
							<slot :name="itemAttr.slot" :row="item" :i="index" :editshow="editShow"></slot>
						</div>
					</td>
				</tr>
